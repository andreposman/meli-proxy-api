...
const rateLimitByIp = (req, res, next) => {
    ...
    try {
        // uncomment to block thereq
        // process.env.BLACK_LISTED_IP = ip

        if (ip != process.env.BLACK_LISTED_IP) {
            if (!client) {
            ...
            client.get(ip, (err, record) => {
                ...
                if (record == null) {
                    ...
                    console.log(`\n Total Reqs in Windows: ${totalWindowRequestsCount}`);
            ...
            res
                .status(ERROR_MESSAGES.BLACK_LISTED.status)
                .jsend.error(ERROR_MESSAGES.BLACK_LISTED.message);
